...
const colorCheck = (red) => (red ? "red" : "gray");
...
export const FigureTemplate = (abce, stepB, stepC, stepE, stepsR, dividers) => `
      ...
      <div id="stepB" class="step stepB karat ${colorCheck(
        isSingle(stepB[0], stepB[1])
      )}">
      ...
      <div id="stepC" class="step stepC karat ${colorCheck(
        isSingle(stepC[0], stepC[1])
      )}">
      ...
      <div id="stepE" class="step stepE karat ${colorCheck(
        isSingle(stepE[0], stepE[1])
      )}">
      ...
      <div class="step__label">G</div>
        ${stepsR[0][0]}<span class="zeros">${trailingZeros(
  dividers[0] * 2
)}</span><br />
        + ${
          stepsR[0][2] - stepsR[0][1] - stepsR[0][0]
        }<span class="zeros">${trailingZeros(dividers[0])}</span><br /> + 
        ${stepsR[0][1]} <hr />
        ${
